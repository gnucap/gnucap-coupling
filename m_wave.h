...
public:
  ...
  WAVE&	   pop_front(double t);
  DPAIR const& back() const { return _w.back(); }
...
/*--------------------------------------------------------------------------*/
inline WAVE& WAVE::pop_front(double t)
{
  assert(_w.begin()!=_w.end());

  auto b=std::next(_w.begin());

  while(b!=_w.end()){ untested();
    if(b->first < t){ untested();
      _w.pop_front();
      b=std::next(_w.begin());
    }else{ untested();
      break;
